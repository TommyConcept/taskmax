...
const supa = {
  ...
  async signUp(email, password, name) {
    const r = await fetch(`${SUPA_URL}/auth/v1/signup`, { method: "POST", headers: { "Content-Type": "application/json", "apikey": SUPA_KEY }, body: JSON.stringify({ email, password, data: { full_name: name } }) });
    return r.json();
  },
  ...
  async getTasks(userId, token) {
    const r = await fetch(`${SUPA_URL}/rest/v1/tasks?user_id=eq.${userId}&order=created_at.asc`, { headers: supa.h(token) });
    return r.json();
  },
  async addTask(task, token) {
    const r = await fetch(`${SUPA_URL}/rest/v1/tasks`, { method: "POST", headers: supa.h(token), body: JSON.stringify(task) });
    const d = await r.json(); return Array.isArray(d) ? d[0] : d;
  },
  async updateTask(id, updates, token) {
    await fetch(`${SUPA_URL}/rest/v1/tasks?id=eq.${id}`, { method: "PATCH", headers: supa.h(token), body: JSON.stringify(updates) });
  },
  async deleteTask(id, token) {
    await fetch(`${SUPA_URL}/rest/v1/tasks?id=eq.${id}`, { method: "DELETE", headers: supa.h(token) });
  },
  async getSub(userId, token) {
    const r = await fetch(`${SUPA_URL}/rest/v1/subscriptions?user_id=eq.${userId}`, { headers: supa.h(token) });
    const d = await r.json(); return Array.isArray(d) ? d[0] : null;
  },
  async upsertSub(sub, token) {
    await fetch(`${SUPA_URL}/rest/v1/subscriptions`, { method: "POST", headers: { ...supa.h(token), "Prefer": "resolution=merge-duplicates,return=representation" }, body: JSON.stringify(sub) });
  }
};
    ...
    try {
      if (mode === "signup") {
        ...
        if (data.access_token) {
          onAuth({ id: data.user.id, email: form.email, name: form.name, token: data.access_token }, "pricing");
        } else {
...
function TaskApp({ user, plan, onUpgrade, onSignOut }) {
  ...
  useEffect(() => {
    supa.getTasks(user.id, user.token).then(d => { setTasks(Array.isArray(d) ? d : []); setLoading(false); });
  }, []);
  ...
  const toggle = async (id, cur) => {
    await supa.updateTask(id, { done: !cur }, user.token);
    setTasks(ts => ts.map(x => x.id === id ? { ...x, done: !cur } : x));
  };

  const del = async (id) => {
    await supa.deleteTask(id, user.token);
          ...
          {!showForm ? (
            ...
            <div style={{ background: "#111420", border: "1px solid #2E3244", borderRadius: 8, padding: "16px" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
                <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} style={{ flex: 1, minWidth: 90, padding: "7px 8px", background: "#0D0F14", border: "1px solid #2E3244", borderRadius: 5, color: "#B0A898", fontFamily: "Georgia,serif", fontSize: 12 }}>
                  {planData.limits.categories.map(c => <option key={c}>{c}</option>)}
                </select>
                <select value={form.priority} onChange={e => setForm(f => ({ ...f, priority: e.target.value }))} style={{ flex: 1, minWidth: 90, padding: "7px 8px", background: "#0D0F14", border: "1px solid #2E3244", borderRadius: 5, color: "#B0A898", fontFamily: "Georgia,serif", fontSize: 12 }}>
...
export default function App() {
  ...
  const handleSignOut = async () => {
    if (user?.token) await supa.signOut(user.token);
    setUser(null); setPlan(null); setBilling(null); setScreen("landing");
  };
